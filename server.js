...
// --- ТВОИ API КЛЮЧИ ---
const AQICN_TOKEN = "f52c21ec384ea8cd25d04e7a6fbdb635dbe5015b"; 
const WEATHER_KEY = "8f6567b813a58b41ed43732d9b807b5c";
...
app.get('/weather', async (req, res) => {
    try {
        const currRes = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${CITY}&appid=${WEATHER_KEY}&units=metric&lang=ru`);
        const curr = await currRes.json();
        
        const foreRes = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${CITY}&appid=${WEATHER_KEY}&units=metric&lang=ru`);
        const fore = await foreRes.json();
        const daily = fore.list ? fore.list.filter(r => r.dt_txt.includes("12:00:00")).slice(0, 3) : [];
        ...
        res.json({
            ...
            forecast: daily.map(f => ({
                date: new Date(f.dt * 1000).toLocaleDateString('ru-RU', { weekday: 'short', day: 'numeric' }),
...
app.get('/pollution-data', async (req, res) => {
    try {
        ...
        const zones = [{n:"Береке", lat:54.885, lon:69.182}, {n:"Рабочий", lat:54.845, lon:69.175}];
        zones.forEach(z => {
            for(let i=0; i<15; i++) {
                stations.push({
                    name: `Датчик ${z.n} #${i+1}`,
        ...
        res.json([{ name: "Демо-точка", lat: 54.867, lon: 69.150, level: 20 }]);
    }
});
...
app.post('/ask', async (req, res) => {
    ...
    const GEMINI_KEY = "AIzaSyDjeZM6qVmGy5uqzCJPnm-BSkZ9-SC-2PE"; 
    ...
    res.json({ reply: "Все доступные модели (Flash, Pro) выдали ошибку 404. Пожалуйста, попробуй создать НОВЫЙ ключ в Google AI Studio, это часто помогает." });
});
